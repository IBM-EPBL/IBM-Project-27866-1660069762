...
try:
    ...
    """
    conn_str='DATABASE=bludb;HOSTNAME=b70af05b-76e4-4bca-a1f5-23dbb4c6a74e.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=32716;SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=qhz63091;PWD=TjpR2GoXknv8F9Xn'
...
def addexpense():
    ...
    while dictionary != False:
        temp = []
        temp.append(dictionary["EXPENSEID"])
    ...
    param="SELECT number from limits ORDER BY limitsid DESC LIMIT 1"
    ...
    if total > int(s):
        ...
        sendmail(session['email'],subject,msg)  
...
def limitnum():
     if request.method == "POST":
         ...
         sql = "INSERT INTO limits (userid, number) VALUES (?, ?)"
         stmt = ibm_db.prepare(ibm_db_conn, sql)
         ibm_db.bind_param(stmt, 1, session['id'])
...
def today():
      ...
      param1="SELECT TIME(date) as tn, amount FROM expenses  WHERE userid = "+str(session['id'])+" AND DATE(date) = DATE(NOW())"
      ...
      param="SELECT * FROM expenses WHERE userid = " + str(session['id']) + " AND DATE(date) = DATE(NOW())"
      res = ibm_db.exec_immediate(ibm_db_conn, param)
      dictionary = ibm_db.fetch_assoc(res)
      expense = []
      while dictionary != False:
